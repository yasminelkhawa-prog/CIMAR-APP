...
interface Props {
  ...
  quickPrompts?: string[];
  /** Display mode: floating button (default) or inline panel */
...
export function FormAssistant({
  ...
  const applyAt = (idx: number) => {
    ...
    if (!msg.suggestions || !onApply) return;
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-3 mb-3 pr-1">
          {messages.length === 0 && (
            <div className="text-center text-muted-foreground text-xs py-8 space-y-2">
              ...
              <p>Demandez à l'IA de vous aider.</p>
          ...
          <div className="flex flex-wrap gap-1.5 mb-2 shrink-0">
            {quickPrompts.map((qp) => (
              <button
                ...
                onClick={() => send(qp)}
                disabled={isLoading}
                className="text-[11px] px-2.5 py-1 rounded-full bg-secondary hover:bg-secondary/80 text-secondary-foreground transition-colors disabled:opacity-50 border border-border"
              >
                {qp}
              </button>
            ))}
